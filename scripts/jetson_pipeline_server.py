...
Usage:
    ...
    python3 scripts/jetson_pipeline_server.py --text "test input"
    python3 scripts/jetson_pipeline_server.py --no-tts
"""
...
class LLM:
    """Generate responses via llama-server HTTP API. No cold start."""
    ...
    def __init__(self):
        ...
        try:
            ...
            print(f"[LLM] Connected to llama-server ({status.get('status', 'ok')})")
        except Exception:
            print(f"[LLM] ERROR: llama-server not running at {LLAMA_SERVER_URL}")
            print(f"[LLM] Start it with:")
            print(f"  ~/llama.cpp/build/bin/llama-server -m ~/models/gemma-4-e4b-it-Q4_K_M.gguf -ngl 99 -c 2048 --port 8080")
            sys.exit(1)
