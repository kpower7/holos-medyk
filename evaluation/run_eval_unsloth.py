"""
Holos Medyk — Benchmark using Unsloth FastModel.
Loads base model + LoRA adapter via Unsloth (handles ClippableLinear).
...
SCENARIOS = [
    ...
    {"id": "shock_02", "lang": "uk", "prompt": "Він притомний але шкіра холодна і мокра, просить пити, пульс дуже швидкий. Що це?", "key_criteria": ["recognize shock", "do not give water", "keep warm"]},
...
# BASELINE
print("=" * 80 + "\nBASELINE: unsloth/gemma-4-E4B-it\n" + "=" * 80, flush=True)
m, tok = FastModel.from_pretrained("unsloth/gemma-4-E4B-it", max_seq_length=2048, load_in_4bit=True)
FastModel.for_inference(m)
baseline = bench(m, tok, "baseline")
del m, tok; gc.collect(); torch.cuda.empty_cache()

# FINE-TUNED — Unsloth auto-detects LoRA adapter and loads base + adapter
print("\n" + "=" * 80 + "\nFINE-TUNED: kevpower/holos-medyk-lora-v5\n" + "=" * 80, flush=True)
m2, tok2 = FastModel.from_pretrained("kevpower/holos-medyk-lora-v5", max_seq_length=2048, load_in_4bit=True)
FastModel.for_inference(m2)
